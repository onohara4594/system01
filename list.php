...
try {
    ...
    if (empty($err)) {
      ...
      $work = $stmt->fetch();
       ...
       if ($work) {
        ...
        //対象日のデータが無ければINSERT
        $sql = "INSERT INTO work (user_id, date, start_time, finish_time, rest_time, comment) VALUES 
        (:user_id, :date, :start_time, :finish_time, :rest_time, :comment)";
        $stmt = $pdo->prepare($sql);
        $stmt->bindValue(':user_id', (int)$session_user['id'], PDO::PARAM_INT);
        $stmt->bindValue(':date', $target_date, PDO::PARAM_STR);
        $stmt->bindValue(':start_time', $modal_start_time, PDO::PARAM_STR);
      ...
      //当日のデータがあるかチェクする
      $sql = "SELECT id, start_time, finish_time, rest_time, comment FROM work WHERE user_id = :user_id AND date = :date LIMIT 1";
      $stmt = $pdo->prepare($sql);
      $stmt->bindValue(':user_id', (int)$session_user['id'], PDO::PARAM_INT);
      ...
      $modal_finish_time = $today_work['finish_time'];
      $modal_rest_time = $today_work['rest_time'];
      $modal_comment = $today_work['comment'];

      if (format_time($modal_start_time) && format_time($modal_finish_time)) {
        $modal_view_flg = FALSE;
      }
    } else {
      $modal_start_time = '';
  ...
  //指定年月の勤怠データを取得
  $sql = "SELECT date, id, user_id, start_time, finish_time, rest_time, comment FROM work WHERE user_id = :user_id AND
          ...
          <div class="modal-body">
            ...
            <div class="container">
              <div class="alert alert-primary" role="alert">
                <span id="modal_month"><?= date('n', strtotime($target_date)) ?></span>/
                <span id="modal_day"><?= time_format_dw($target_date) ?></span>
              </div>
              <div class="row">
                <div class="col">
                  <div class="input-group">
                    <input type="text" class="form-control <?php if (isset($err['modal_start_time'])) echo ' is-invalid'; ?>" 
                    placeholder="出勤" id="modal_start_time" name="modal_start_time" value="<?= format_time($modal_start_time) ?>" required>
                ...
                <div class="col">
                  <div class="input-group">
                    <input type="text" class="form-control <?php if (isset($err['modal_finish_time'])) echo ' is-invalid'; ?>" 
                    placeholder="退勤" id="modal_finish_time" name="modal_finish_time" value="<?= format_time($modal_finish_time) ?>">
                    <div class="input-group-prepend">
                ...
                <div class="col">
                  <input type="text" class="form-control <?php if (isset($err['modal_rest_time'])) echo ' is-invalid'; ?>" 
                  placeholder="休憩" id="modal_rest_time" name="modal_rest_time" value="<?= format_time($modal_rest_time) ?>">
                  <div class="invalid-feedback"><?= $err['modal_rest_time'] ?></div>
                </div>
              </div>
              <div class="form-floating pt-3">
                <textarea class="form-control <?php if (isset($err['modal_comment'])) echo ' is-invalid'; ?>" id="modal_comment" 
                name="modal_comment" placeholder="業務内容"><?= $modal_comment ?></textarea>
